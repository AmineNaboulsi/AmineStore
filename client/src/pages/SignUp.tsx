...
function SignUp() {
  const navigate = useNavigate();
    ...
    const SignUp = (formData:FormData) =>{
          ...
          fetch("http://localhost:8484/signup", {
              method: "POST",
              body: formData,
          })
          .then((response) => {
          ...
          <form onSubmit={HandleSignIn}>
              ...
              <div>
                ...
                <div className="relative flex items-center">
                  <input name="name" type="text"  className="w-full text-gray-800 text-sm border-b border-gray-300 focus:border-blue-600 pl-2 pr-8 py-3 outline-none" placeholder="Enter name" />
                </div>
              </div>
              <div className="mt-8">
                <label className="text-gray-800 text-xs block mb-2">Email</label>
                <div className="relative flex items-center">
                  <input name="email" type="text"  className="w-full text-gray-800 text-sm border-b border-gray-300 focus:border-blue-600 pl-2 pr-8 py-3 outline-none" placeholder="Enter password" />
                </div>
              </div>
              <div className="mt-8">
              ...
              {(loading.message != '') && 
              (
                <div className={`mt-5 flex items-center p-3  text-sm ${loading.status ? 'bg-green-200': 'bg-red-100'} rounded-lg   ${loading.status ? 'dark:text-green-700': 'dark:text-red-400'}`} role="alert">
